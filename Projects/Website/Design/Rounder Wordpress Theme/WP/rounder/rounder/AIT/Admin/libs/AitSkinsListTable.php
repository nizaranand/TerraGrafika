	...
	function display_rows()
	{
		...
		if(!empty($skin)){
			if(!empty($skins[$skin]['theme']) and $skins[$skin]['theme'] != THEME_CODE_NAME){
				...
				echo "<h3>" . sprintf( __( '%1$s by %2$s' ), $skins[$skin]['name'], $skins[$skin]['author']) . "</h3>";
				?><p class="description" style="color:crimson;"><?php _e('This skin is not for this theme'); ?></p><?php
				continue;
			}
			...
			<p><?php printf( __( 'All of this theme&#8217;s files are located in<br><code>%2$s</code>.' ), $skins[$skin]['name'], str_replace(THEME_URL, '', $skins[$skin]['url'])); ?></p>
